...
// 丸十支出記帳（#支出）的 Sheet 寫入與同步，獨立成檔（見 api/maruten-expense.js）。
...
// 把某筆記帳對應的 Sheet 列號寫回 xlan_expenses（供日後改分類／刪除同步該列）。
// 失敗只記 log、不拋錯——Sheet 同步是加值功能，不能因此影響主流程或讓使用者收到錯誤。
// 回傳 true=寫回成功；false=失敗（欄位未部署／RLS／update 失敗）。呼叫端據此在卡片提示，
// 讓使用者知道「列號回寫失敗」（之後改分類／刪除會退回掃 Sheet 找 ID），不誤以為一切就緒（P2-3）。
async function setExpenseSheetRow(expenseId, sheetRow) {
  if (!expenseId || !Number.isFinite(Number(sheetRow))) return false;
  try {
    const { error } = await supabase
      .from('xlan_expenses')
      .update({ sheet_row: Number(sheetRow) })
      .eq('id', expenseId);
    if (error) {
      console.error('set_expense_sheet_row_error', error.message);
      return false;
    }
    return true;
  } catch (e) {
    console.error('set_expense_sheet_row_error', e?.message);
    return false;
  }
}
...
async function deleteExpense(expenseId, sourceKey = '') {
  // 先讀出該筆（不刪），判斷是否為丸十支出，決定刪除順序。
  const { data: target } = await supabase
    .from('xlan_expenses')
    .select('*')
    .eq('id', expenseId)
    .single();
  if (!target) return '找不到那筆記帳，可能已經刪掉了。';

  // 丸十支出：先標 Sheet「已刪除」成功，再刪 DB（P1-2）。
  // 理由：若先刪 DB 再標 Sheet，Google API 失敗會讓 Sheet 仍是「正常」而 DB 已無此筆，月底以 Sheet 看會多算。
  // 反過來「先標 Sheet」即使之後刪 DB 失敗，最壞只是該筆在 DB 還在（會被下次刪除/人工處理），不會造成「已刪卻被加總」。
  // 非丸十（含 entity 為 null 的舊私訊／既有記帳）完全不走這段，行為不變。
  if (target.entity === '丸十') {
    let sheetMarked = false;
    try {
      sheetMarked = await syncMarutenSheetOnDelete(target);
    } catch (e) {
      console.error('maruten_sheet_sync_delete_error', e?.message);
      sheetMarked = false;
    }
    if (!sheetMarked) {
      return `⚠️ 這筆是丸十支出，但同步「丸十支出表」標記刪除失敗，為避免月底多算，暫不刪除。請稍後再試或到後台確認。`;
    }
  }
  ...
  const { data, error } = await supabase
    ...
    .eq('id', expenseId)
    .select();
  // DB 刪除失敗：此前若已標 Sheet 為已刪除，必須回滾，否則「Sheet 已刪、DB 還在」會在查詢／摘要造成歧義（P1-B）。
  if (error) {
    if (target.entity === '丸十') {
      let rolledBack = false;
      try {
        rolledBack = await rollbackMarutenSheetDelete(target);
      } catch (e) {
        console.error('maruten_sheet_rollback_error', e?.message);
        rolledBack = false;
      }
      if (!rolledBack) {
        // 兩邊都沒能對齊（DB 沒刪、Sheet 還原也失敗）：明確要求人工處理，避免靜默不一致。
        return `⚠️ 刪除失敗（${error.message}），且「丸十支出表」那列已標記為刪除卻無法自動還原，請到後台確認該列狀態。`;
      }
      return `刪除失敗（${error.message}），已把「丸十支出表」標記還原，這筆仍然存在。請稍後再試一次。`;
    }
  ...
  if (stillThere && stillThere.length > 0) {
    // DB delete 沒回 error，但後置驗證發現資料其實還在（靜默未刪）：與「delete 回 error」同樣是
    // 「Sheet 已標已刪、DB 還在」的不一致，必須一併回滾丸十 Sheet 標記，否則月底以 Sheet 看會少算（P1-B）。
    if (target.entity === '丸十') {
      let rolledBack = false;
      try {
        rolledBack = await rollbackMarutenSheetDelete(target);
      ...
      if (!rolledBack) {
        return `⚠️ 嘗試刪除「${deleted.category} NT$${deleted.amount}」但它還在，且「丸十支出表」那列已標記為刪除卻無法自動還原，請到後台確認該列狀態。`;
      }
      return `⚠️ 嘗試刪除「${deleted.category} NT$${deleted.amount}」但它還在，已把「丸十支出表」標記還原，這筆仍然存在。請再試一次或到後台確認。`;
    }
...
async function updateLatestExpenseCategory(category) {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) throw new Error(error.message);
  await syncMarutenSheetOnCategory(data);
...
// 若這筆是丸十支出，把改後的分類同步到丸十支出 Sheet 的該列。失敗只記 log、不影響回覆。
// 限定 entity === '丸十'：未來其他主體（央廚／匯洲／全民）有各自的 Sheet 路由，不可誤跑丸十表；
// entity 為 null 的舊私訊／既有記帳更不會進來，行為完全不變（P1-3）。
async function syncMarutenSheetOnCategory(expense) {
  if (expense?.entity !== '丸十') return;
  try {
    await marutenExpense.updateSheetCategory(supabase, {
      sheetRow: expense.sheet_row,
      expenseId: expense.id,
      category: expense.category,
    });
  } catch (e) {
    console.error('maruten_sheet_sync_category_error', e?.message);
  }
}

// 若這筆是丸十支出，在丸十支出 Sheet 把該列標記為已刪除。
// 回傳 true=標記成功；false=非丸十或標記失敗。呼叫端（deleteExpense）據此決定是否續刪 DB（P1-2）。
// 限定 entity === '丸十'，理由同 syncMarutenSheetOnCategory（P1-3）。
async function syncMarutenSheetOnDelete(expense) {
  if (expense?.entity !== '丸十') return false;
  return marutenExpense.markSheetDeleted(supabase, {
    sheetRow: expense.sheet_row,
    expenseId: expense.id,
  });
}

// 回滾 syncMarutenSheetOnDelete 的標記（DB 刪除失敗時用，P1-B）：把 Sheet 那列從「已刪除」還原成「正常」，
// 確保「Sheet 已標已刪、DB 還在」這種不一致狀態不會殘留。回傳 true=已還原；false=非丸十或還原失敗。
async function rollbackMarutenSheetDelete(expense) {
  if (expense?.entity !== '丸十') return false;
  return marutenExpense.restoreSheetDeleted(supabase, {
    sheetRow: expense.sheet_row,
    expenseId: expense.id,
  });
}
...
// ====================== 丸十支出記帳（#支出 分支）======================
// 需求單／實作計畫：D:\丸十支出機器人\。掛在 handleGroupMessage 的 # 指令鏈、隱式記帳之前。

// --- 任務3：群組 → 主體對應 ---
// kv key「group_entity_map」存 JSON：{ "<groupId>": "丸十", ... }，未設定的群組回 null。
// 設計成可設定而非寫死，為未來央廚／匯洲／全民四處對等預留。
const KV_GROUP_ENTITY_MAP = 'group_entity_map';
...
// 「支出」之後若接的是「純查詢詞」，視為查詢（看明細／看表）而非記帳；後面頂多再接數字（如「明細 100」表示近 100 筆）。
// 查詢詞清單：完整詞比對（不是首字攔截），所以「表格紙」「統計費」這類品項名不會被誤判成查詢（P2-1）。
const EXPENSE_QUERY_WORDS = ['明細', '表', '清單', '查詢', '統計', '報表', '總計', '彙總'];
// 句尾可帶的「位置疑問詞」：允許「支出表在哪」「支出在哪」這類問句仍判為查詢。
const EXPENSE_QUERY_LOCATION_SUFFIX = ['在哪裡', '在哪', '哪裡'];

// 判斷「支出」之後是不是一個「純查詢詞」（可選帶位置疑問詞、可選再帶一個數字）。
// 作法：strip 掉開頭「支出」後，切出第一個「詞」（連續非空白且非數字的字元），
// 該詞必須「完整等於查詢詞」或「去掉句尾位置疑問詞後完整等於查詢詞（或為空）」才算查詢。
//   - 「明細」「支出明細」「明細 100」→ 第一詞「明細」∈ 清單 → 查詢
//   - 「表在哪」→ 去尾綴「在哪」剩「表」∈ 清單 → 查詢；「在哪」→ 去尾綴剩空 → 查詢
//   - 「表格紙 100」→ 第一詞「表格紙」不在清單、去尾綴也不符 → 不是查詢（記帳）
function isExpenseQueryText(body) {
  let s = String(body || '').trim();
  s = s.replace(/^支出[\s:：]*/, '').trim();   // 去掉「支出」前綴
  if (s === '') return false;                  // 只有「支出」本身 → 交給用法提示分支，不在此判查詢
  // 第一個詞：開頭連續的非空白、非數字字元（中文詞段）。數字／空白即詞邊界。
  const m = s.match(/^[^\s\d]+/);
  if (!m) return false;                         // 開頭就是數字（如「支出 100」）→ 不是查詢，是記帳
  let token = m[0];
  // 詞後面只允許是「空白＋數字」或結尾；若黏著其他文字（如「明細表單之類」）才算詞的一部分已被吃進 token。
  const rest = s.slice(token.length).trim();
  if (rest !== '' && !/^\d[\d,]*(\s*元)?$/.test(rest)) return false;  // 詞後還有非數字內容 → 偏向品項，不當查詢
  if (EXPENSE_QUERY_WORDS.includes(token)) return true;
  // 去掉句尾位置疑問詞再比對一次（「表在哪」「在哪」）。
  for (const suf of EXPENSE_QUERY_LOCATION_SUFFIX) {
    if (token.endsWith(suf)) {
      const head = token.slice(0, token.length - suf.length);
      if (head === '' || EXPENSE_QUERY_WORDS.includes(head)) return true;
    }
  }
  return false;
}
...
// --- 任務4：解析 #支出 文字 ---
// 輸入是「已被 stripGroupHashTrigger 去掉 #、但仍含『支出』前綴」的字串，例如「支出 便當 120」。
// 強制視為支出（type=expense），不依賴隱式記帳那套收支訊號偵測。
// 回傳 { amount, category, note } 或 null（無金額／空項目）。
function parseMarutenExpenseText(text) {
  let raw = String(text || '').trim();
  if (!raw) return null;
  // 去掉開頭的「支出」前綴詞（可能後面接空白、冒號或直接接內容），避免把「支出」留進項目或分類。
  raw = raw.replace(/^支出[\s:：]*/, '').trim();
  if (!raw) return null;

  // 抽金額：支援 1,234 / NT$120 / 120元 等寫法，取最後一個數字當金額（項目在前、金額在後較自然）。
  ...
  // 項目 = 原字串扣掉金額那段文字，去頭尾雜符。
  // 金額 token（last[0]）已含其尾綴「元」（amountRe 的 (?:\s*元)?），故切掉金額段時「元」一併移除；
  // 不再對整個項目做 .replace(/元/g,'')，否則「元氣早餐 120」會被截成「氣早餐」（P2-2）。
  const note = raw.slice(0, last.index).concat(raw.slice(last.index + last[0].length))
    .replace(/[，,、:：]+$/g, '')
    .trim();
  const item = note || raw.replace(last[0], '').trim() || '支出';
  const category = inferExpenseCategory(item, 'expense');
  return { amount, category, note: item };
}

// 判斷一則原始群組訊息是不是 #支出 記帳指令。
// 條件：以「#支出／＃支出」開頭，且 ——
//   (a) 「支出」後面緊接空白／冒號／結尾（明確的 #支出 指令，含只打「#支出」想看用法），或
//   (b) 去掉 # 後能被 parseMarutenExpenseText 解析成功（涵蓋「#支出便當120」這種無空白黏著、但確實是記帳的）。
// 這樣可精準排除「#支出表在哪」「#支出明細」這類含「支出」開頭卻無金額的查詢句。
function isMarutenExpenseTrigger(text) {
  const raw = String(text || '').trim();
  if (!/^[#＃]\s*支出/.test(raw)) return false;
  // 排除「支出表／支出明細／支出清單…」這類查詢句，避免帶數字時把「表在哪／明細」記成項目（P2-1）。
  // 例：「#支出明細 100」「#支出表在哪 120」不應走記帳，往下交給其他分支或一般處理。
  const body = stripGroupHashTrigger(raw).replace(/@\S+/g, '').trim();
  // 查詢句（看明細／看表）不走記帳：用「完整查詢詞」token 比對，避免「支出 表格紙 100」因首字「表」被誤殺（P2-1）。
  if (isExpenseQueryText(body)) return false;
  if (/^[#＃]\s*支出(?:[\s:：]|$)/.test(raw)) return true;
...
function buildMarutenExpenseFlex({ entity, category, note, amount, recorder, dateText, sheetWarning, balance }) {
  ...
  return {
    type: 'flex',
    altText: `${entity || '丸十'}支出 ${category} NT$${amount}`,
    contents: {
      type: 'bubble',
      size: 'kilo',
      body: {
            ...
            contents: [
              {
                ...
                height: 'sm',
                style: 'secondary',
                action: { type: 'message', label: '改分類', text: '改分類' },
              },
              {
                type: 'button',
                height: 'sm',
                color: CARD_THEME.danger,
                style: 'primary',
                action: { type: 'message', label: '刪除這筆', text: '刪掉' },
              },
            ],
          },
        ],
...
// --- 切片二：丸十支出 LIFF 表單入口 ---
// 取得有效的丸十支出 LIFF ID：env 有設且非空白、且不是佔位值才算「真的有設定」。
// 回傳真 LIFF ID 或 ''（沒設／還是佔位 → 走打字用法提示，不發無效卡片）。
function getMarutenExpenseLiffId() {
  const liffId = (MARUTEN_EXPENSE_LIFF_ID || '').trim();
  if (!liffId || liffId === MARUTEN_EXPENSE_LIFF_PLACEHOLDER) return '';
  return liffId;
}

// 組丸十支出表單的 LIFF URL（仿 buildStaffReportLiffUrl）：endpoint 對應 maruten-expense.html，帶 groupId。
function buildMarutenExpenseLiffUrl(source) {
  ...
  return {
    ...
    contents: {
      ...
      body: {
        ...
        spacing: 'md',
        contents: [
          { type: 'text', text: '丸十支出', weight: 'bold', size: 'lg', color: CARD_THEME.primaryDark },
          { type: 'text', text: '選分類、填金額、拍收據，送出就寫進支出表。', size: 'sm', color: CARD_THEME.muted, wrap: true },
          {
            type: 'box',
            layout: 'vertical',
            ...
            spacing: 'xs',
            contents: [
              { type: 'text', text: '1. 點下面「開支出表單」', size: 'sm', color: CARD_THEME.text, wrap: true },
              { type: 'text', text: '2. 選分類、填項目與金額，可拍收據存證', size: 'sm', color: CARD_THEME.text, wrap: true },
      ...
      footer: {
        ...
        spacing: 'sm',
        contents: [
          { type: 'button', style: 'primary', color: CARD_THEME.primary, height: 'sm', action: { type: 'uri', label: '📋 開支出表單', uri: liffUrl } },
        ],
      },
    },
  };
...
async function handleMarutenExpense(event, cleanedText, focusKey) {
  ...
  // 先存 Supabase（主體＝entity、強制 expense、帳別歸 business）。這步失敗才真的算記帳失敗。
  let savedExpense;
  try {
    savedExpense = await saveExpense({
      amount: parsed.amount,
      category: parsed.category,
      note: parsed.note,
      type: 'expense',
    ...
    console.error('maruten_expense_save_error', e?.message);
    return [{ type: 'text', text: `記帳失敗了（${e?.message || '資料庫錯誤'}），請稍後再試一次。` }];
  }

  // 設成「最近一筆」焦點 → 沿用既有「改分類／刪掉」修正機制（resolveFocusedExpenseReply）。
  await saveExpenseFocus(focusKey, savedExpense);

  // 再寫進「丸十支出」Google Sheet。失敗不影響已存的 Supabase 資料，只在卡片附提示。
  let sheetWarning = '';
  try {
    ...
    if (!sheetRow) {
      // append 沒回列號（解析 updatedRange 失敗等）：Sheet 可能寫了但列號未知，後續同步要靠掃 ID。
      sheetWarning = '⚠️ 已記到資料庫，但無法確認丸十支出表的列號，改分類／刪除時會以記帳ID比對（稍後可在後台確認）。';
    } else {
      // 回寫列號失敗：append 已成功（Sheet 有資料），只是 sheet_row 沒存進 DB；明確告知，別讓使用者以為一切就緒（P2-3）。
      const rowSaved = await setExpenseSheetRow(savedExpense.id, sheetRow);
      if (!rowSaved) {
        sheetWarning = '⚠️ 已記到資料庫並寫入丸十支出表，但列號回寫失敗，之後改分類／刪除會以記帳ID比對（如反覆出現請通知管理員）。';
      }
    }
  } catch (e) {
    console.error('maruten_expense_sheet_error', e?.message);
...
// #補錢 後若緊接這些「純查詢詞」（且整段不含數字），視為查詢句（看明細／看表）而非補錢記帳，不觸發（沿用 #支出 的查詢詞慣例）。
// 判斷邏輯在 isMarutenTopupTrigger：rest 以查詢詞為「前綴」且「整段不含數字」才排除（詳見該函式內註解）；
// 故品項備註（如「現金」「六月零用金」）、或以查詢詞開頭但後接金額者（「明細 100」），都不會被誤判成查詢。
...
async function handleGroupMessage(event) {
  ...
  if (msgType === 'text') {
    ...
    if (hasHashTrigger) {
      ...
      if (isMarutenExpenseTrigger(text)) {
        const expenseText = stripGroupHashTrigger(text).replace(/@\S+/g, '').trim();
...
// 比照 maruten-expense.js 的 _internal 慣例；production 路徑不使用這個屬性。
// P2：只在 NODE_ENV==='test' 時掛上，避免污染 production module surface。
// 測試端（test/_stubs.js）會在 require 本模組前設好 NODE_ENV=test，故 node --test 仍取得到 __test__。
if (process.env.NODE_ENV === 'test') {
  module.exports.__test__ = {
    parseMarutenExpenseText,
    isMarutenExpenseTrigger,
    inferExpenseCategory,
    // 流程層（測 P0／P1-2／P1-3，需搭配 test/_stubs 注入假 supabase 與假 maruten 模組）
    handleMarutenExpense,
    deleteExpense,
    syncMarutenSheetOnCategory,
    syncMarutenSheetOnDelete,
    // 切片二：LIFF 表單入口（卡片分流）
